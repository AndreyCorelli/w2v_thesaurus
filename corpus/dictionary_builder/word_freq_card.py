    ...
    def calc_stat(cls, words: List[str]) -> Dict[str, 'WordFrequencyCard']:
        ...
        for w in words:
            ...
            if card.last_index >= 0:
                card.distances.append(index - card.last_index - 1)
        ...
        for w in wrd_cards:
            ...
            if card.frequency > 1:
                avg_dist = (len(words) - 1) / (card.frequency - 1) - 1
                card.nu = sum([(d-avg_dist)**2 for d in card.distances]) / len(card.distances)
            else:
                card.nu = 10000  # this is the only entry
